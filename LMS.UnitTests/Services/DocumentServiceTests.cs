...
namespace LMS.UnitTests.Services
{
    public class DocumentServiceTests : ServiceTestBase
    {
        ...
        public async Task GetDocumentByIdAsync_DocumentDoesNotExist_ThrowsDocumentNotFoundException()
        {
            ...
            await Assert.ThrowsAsync<DocumentNotFoundException>(() => _service.GetDocumentByIdAsync(documentId));

            MockDocumentRepo.Verify(r => r.GetDocumentByIdAsync(documentId), Times.Once);
            MockMapper.Verify(m => m.Map<DocumentDto>(It.IsAny<Document>()), Times.Never);
        }
        #endregion
        ...
        public async Task CreateDocumentAsync_ValidDto_CreatesAndReturnsEntity()
        {
            ...
            MockFileHandlerService
                .Setup(f => f.UploadFileAsync(
                    It.IsAny<Stream>(), 
                    It.IsAny<string>(), 
                    It.IsAny<string>()))
                .ReturnsAsync("dummy/path/NewDoc.txt");
                        
            MockMapper
                .Setup(m => m.Map<DocumentDto>(mappedEntity))
            ...
            var dummyStream = new MemoryStream(Encoding.UTF8.GetBytes("dummy content"));
            var result = await _service.CreateDocumentAsync(createDto, dummyStream);
            ...
            MockFileHandlerService.Verify(f => f.UploadFileAsync(
                dummyStream, 
                It.IsAny<string>(), 
                It.IsAny<string>()), 
                Times.Once);
        ...
        public async Task UpdateDocumentAsync_ValidDto_UpdatesAndReturnsEntity()
        {
            ...
            MockDocumentRepo    
                .Setup(r => r.GetDocumentByIdAsync(dto.Id))
